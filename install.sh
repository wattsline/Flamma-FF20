...
section() { printf "\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"; bold "$*"; printf "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"; }
run_or_fail() { "$@"; local rc=$?; [[ $rc -eq 0 ]] || fail "Command failed: $*"; }
...
install_homebrew_if_missing() {
  load_homebrew_env
  if command -v brew >/dev/null 2>&1; then ok "Homebrew found: $(brew --version | head -1)"; return; fi
...
install_brew_package() {
  ...
  else echo "Installing $pkg..."; run_or_fail brew install "$pkg"; ok "$pkg installed"; fi
}
...
create_venv_and_install() {
  ...
  [[ -f "pyproject.toml" ]] || fail "pyproject.toml not found. Run install.sh from the project directory."
  if [[ ! -d ".venv" ]]; then run_or_fail "$PYTHON_BIN" -m venv .venv; ok "Virtual environment created";
  else ok "Virtual environment already exists"; fi
  VENV_PYTHON="$(find_venv_python)"
  [[ -n "$VENV_PYTHON" ]] || fail "Could not find .venv/bin/python3 or .venv/bin/python"
  run_or_fail "$VENV_PYTHON" -m pip install --upgrade pip
  run_or_fail "$VENV_PYTHON" -m pip install -e ".[gui]"
...
smoke_test() {
  section "Smoke test"
  VENV_PYTHON="$(find_venv_python)"
  [[ -n "$VENV_PYTHON" ]] || fail "Could not find .venv Python"
  run_or_fail "$VENV_PYTHON" -m ff20.cli --help >/dev/null
  ok "CLI module available"
  if .venv/bin/ff20 devices 2>/dev/null | grep -q "FF20"; then ok "FF20 pedal detected";
  else warn "No FF20 pedal detected. This is OK if the pedal is not connected."; fi
  .venv/bin/ff20 doctor || warn "Doctor completed with warnings."
}
...
create_app_launcher() {
  ...
  answer="${answer:-Y}"
  if [[ "$answer" =~ ^[Yy]$ ]]; then
    [[ -x "./create_macos_app.sh" ]] && run_or_fail ./create_macos_app.sh || warn "create_macos_app.sh not found."
  else warn "Skipping macOS app launcher."; fi
}
